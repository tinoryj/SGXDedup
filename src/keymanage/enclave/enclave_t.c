...
typedef struct ms_ecall_keygen_t {
	sgx_status_t ms_retval;
	sgx_ra_context_t* ms_ctx;
	sgx_ra_key_type_t ms_type;
	uint8_t* ms_batchHash;
	uint32_t ms_srcLen;
	uint8_t* ms_batchKey;
} ms_ecall_keygen_t;
...
static sgx_status_t SGX_CDECL sgx_ecall_keygen(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_keygen_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_keygen_t* ms = SGX_CAST(ms_ecall_keygen_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_ra_context_t* _tmp_ctx = ms->ms_ctx;
	size_t _len_ctx = sizeof(sgx_ra_context_t);
	sgx_ra_context_t* _in_ctx = NULL;
	uint8_t* _tmp_batchHash = ms->ms_batchHash;
	uint32_t _tmp_srcLen = ms->ms_srcLen;
	size_t _len_batchHash = _tmp_srcLen;
	uint8_t* _in_batchHash = NULL;
	uint8_t* _tmp_batchKey = ms->ms_batchKey;
	size_t _len_batchKey = 16;
	uint8_t* _in_batchKey = NULL;

	CHECK_UNIQUE_POINTER(_tmp_ctx, _len_ctx);
	CHECK_UNIQUE_POINTER(_tmp_batchHash, _len_batchHash);
	CHECK_UNIQUE_POINTER(_tmp_batchKey, _len_batchKey);
	...
	if (_tmp_ctx != NULL && _len_ctx != 0) {
		_in_ctx = (sgx_ra_context_t*)malloc(_len_ctx);
		if (_in_ctx == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_ctx, _len_ctx, _tmp_ctx, _len_ctx)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_batchHash != NULL && _len_batchHash != 0) {
		_in_batchHash = (uint8_t*)malloc(_len_batchHash);
		if (_in_batchHash == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_batchHash, _len_batchHash, _tmp_batchHash, _len_batchHash)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_batchKey != NULL && _len_batchKey != 0) {
		if ((_in_batchKey = (uint8_t*)malloc(_len_batchKey)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_batchKey, 0, _len_batchKey);
	}

	ms->ms_retval = ecall_keygen(_in_ctx, ms->ms_type, _in_batchHash, _tmp_srcLen, _in_batchKey);
err:
	if (_in_ctx) free(_in_ctx);
	if (_in_batchHash) free(_in_batchHash);
	if (_in_batchKey) {
		if (memcpy_s(_tmp_batchKey, _len_batchKey, _in_batchKey, _len_batchKey)) {
			status = SGX_ERROR_UNEXPECTED;
		}
		free(_in_batchKey);
	}
...
SGX_EXTERNC const struct {
	...
	6,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_keygen, 0},
