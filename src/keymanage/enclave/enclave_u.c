...
sgx_status_t ecall_keygen(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t* ctx, sgx_ra_key_type_t type, uint8_t* batchHash, uint32_t srcLen, uint8_t* batchKey)
{
	sgx_status_t status;
	ms_ecall_keygen_t ms;
	ms.ms_ctx = ctx;
	ms.ms_type = type;
	ms.ms_batchHash = batchHash;
	ms.ms_srcLen = srcLen;
	ms.ms_batchKey = batchKey;
