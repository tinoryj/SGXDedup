...
typedef struct ms_ecall_calcmac_t {
	sgx_status_t ms_retval;
	sgx_ra_context_t* ms_ctx;
...
static sgx_status_t SGX_CDECL sgx_ecall_calcmac(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_calcmac_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_calcmac_t* ms = SGX_CAST(ms_ecall_calcmac_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_ra_context_t* _tmp_ctx = ms->ms_ctx;
	size_t _len_ctx = sizeof(sgx_ra_context_t);
	sgx_ra_context_t* _in_ctx = NULL;
	...
	size_t _len_src = _tmp_srcLen;
	uint8_t* _in_src = NULL;
	uint8_t* _tmp_cmac = ms->ms_cmac;
	size_t _len_cmac = 128;
	uint8_t* _in_cmac = NULL;

	CHECK_UNIQUE_POINTER(_tmp_ctx, _len_ctx);
	CHECK_UNIQUE_POINTER(_tmp_src, _len_src);
	CHECK_UNIQUE_POINTER(_tmp_cmac, _len_cmac);
	...
	if (_tmp_src != NULL && _len_src != 0) {
		_in_src = (uint8_t*)malloc(_len_src);
		if (_in_src == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_src, _len_src, _tmp_src, _len_src)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_cmac != NULL && _len_cmac != 0) {
		if ((_in_cmac = (uint8_t*)malloc(_len_cmac)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_cmac, 0, _len_cmac);
	}

	ms->ms_retval = ecall_calcmac(_in_ctx, ms->ms_type, _in_src, _tmp_srcLen, _in_cmac);
err:
	if (_in_ctx) free(_in_ctx);
	if (_in_src) free(_in_src);
	if (_in_cmac) {
		if (memcpy_s(_tmp_cmac, _len_cmac, _in_cmac, _len_cmac)) {
			status = SGX_ERROR_UNEXPECTED;
		}
		free(_in_cmac);
	}
...
SGX_EXTERNC const struct {
	...
	6,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_calcmac, 0},
