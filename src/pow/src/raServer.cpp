...
bool raServer::get_attestation_report(const char *b64quote, sgx_ps_sec_prop_desc_t secprop, ra_msg4_t *msg4) {
...
bool raServer::get_sigrl(uint8_t *gid, char **sig_rl, uint32_t *sig_rl_size) {
    IAS_Request *req = nullptr;
...
bool raServer::derive_kdk(EVP_PKEY *Gb, unsigned char *kdk, sgx_ec256_public_t g_a) {
    unsigned char *Gab_x;
...
bool raServer::process_msg01(powSession *session, sgx_msg01_t &msg01, sgx_ra_msg2_t &msg2) {
...
bool raServer::process_msg3(powSession *session, sgx_ra_msg3_t *msg3, ra_msg4_t &msg4, uint32_t quote_sz) {
    if (CRYPTO_memcmp(&msg3->g_a, &session->msg1.g_a,
                      sizeof(sgx_ec256_public_t))) {
        cerr << "msg1.ga != msg3.ga\n";
        return false;
    }
    sgx_mac_t msgMAC;
    cmac128(session->smk, (unsigned char *) &msg3->g_a, sizeof(sgx_ra_msg3_t) -
    ...
    if (memcmp(session->msg1.gid, &q->epid_group_id, sizeof(sgx_epid_group_id_t))) {
